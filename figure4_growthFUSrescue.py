...
growth = pd.read_csv('../data/merged_growthrates.csv')
...
plt.xlim(90, 300)
plt.tight_layout()
plt.savefig('../output/Fig4_growthrates_FUS.svg')
...
plt.savefig('../output/Fig4_RNAseqHmap.svg')
...
plt.savefig('../output/Fig4_RNAseqLolipop.svg')
...
for coef, clabel in zip(coefs, coef_labels):
    ...
    plt.savefig('../output/Fig4_RNAseqScatter_Int_{}.svg'.format(clabel.split(' ')[0]))
...
for coef, clabel in zip(coefs, coef_labels):
    ...
    plt.savefig('../output/Fig4_RNAseqScatter_GalOnly_{}.svg'.format(clabel.split(' ')[0]))
...
for gene, _freq_df in freq_df.groupby('gene'):
    ...
    plt.savefig('../output/Fig4_FracActive_{}.svg'.format(gene))
...
#            for strain, vals in _parts.groupby('strain')]
    plt.tight_layout()
    plt.savefig('../output/Fig4_ECDF_{}.svg'.format(gene))
